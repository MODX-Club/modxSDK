...
class modxSDKMgrVaporSnapshotCreateProcessor extends modProcessor{
    
    public $permission = 'Vapor';
    ...
    public function initialize(){
        ...
        $this->setDefaultProperties(array(
            ...
            'includeExtensionPackages' => false, 
            'sources'   => array(),
        ));
        
        
        if(!$PKG_NAME = $this->getProperty('pkg_name')){
            return 'pkg_name not set';
        }
        if(!$PKG_VERSION = $this->getProperty('pkg_version')){
            return 'pkg_version not set';
        }
        if(!$PKG_RELEASE = $this->getProperty('pkg_release')){
            return 'pkg_release not set';
        }
        ...
        define('BASE_DIR', $this->modx->getOption('base_path'));
        define('VAPOR_DIR', $this->modx->getOption('core_path').'components/modxsdk/model/vapor/');
    ...
    public function process(){
        ...
        try {
            $vaporOptions = array(
                ...
                'excludeFiles' => array(
            		MODX_BASE_PATH .'vapor',
        			MODX_BASE_PATH .'phpmyadmin',
        			MODX_BASE_PATH .'assets',
        			MODX_BASE_PATH .'core',
        		)
            );
            ...
            $options = array(
                ...
                'log_target' => array(
                    'target' => 'FILE',
                    'options' => array(
                        'filename' => 'vapor-' . strftime('%Y%m%dT%H%M%S', $this->getProperty('startTime')) . '.log'
                    )
                ),
            ...
            foreach($this->getProperty('sources') as $source_id){
                ...
                if ($dh = opendir(MODX_BASE_PATH . $path)) {
                    ...
                    while (($file = readdir($dh)) !== false) {
                        ...
                        $includes[] = array(
                            'source' => MODX_BASE_PATH . $path . $file,
                            'target' => "return MODX_BASE_PATH . '{$path}/';"
                        );
                    }
                    closedir($dh);
                    foreach ($includes as $include) {
    ...
    protected function getClassesList(){
        ...
        if($this->getProperty('includeBaseClasses')){
            $baseClasses = $this->getBaseClassesList();
    ...
    protected function getBaseClassesList(){
        $classes =  array (
                'modAccessAction',
                'modAccessActionDom',
                'modAccessCategory',
    ...
    public function getSource($source_id) {
        ...
        if(!$this->source = $this->modx->getObject('sources.modMediaSource',array(
            'id' => $source_id,
        ))){
            return false;
        }
...
return 'modxSDKMgrVaporSnapshotCreateProcessor';